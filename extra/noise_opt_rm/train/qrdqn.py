"""QR-DQN training with discrete noise candidates for robomimic."""
import math
import os
import random
import sys
...
from noise_opt_rm.utils.env_utils import (ActionChunkWrapper, DiffusionPolicyEnvWrapper,
                                     ...
                                     make_robomimic_env)
from noise_opt_rm.train.train_utils.qrdqn_callbacks import QRDQNNoiseLoggingCallback
...
@hydra.main(config_path=os.path.join(base_path, "cfg/robomimic/qrdqn"), config_name="can.yaml", version_base=None)
def main(cfg: OmegaConf):
    ...
    model = QRDQN(
        ...
        exploration_initial_eps=cfg.train.exploration_initial_eps,
    ...
    logging_callback = QRDQNNoiseLoggingCallback(
    ...
    total_timesteps = cfg.train.total_timesteps if hasattr(cfg.train, 'total_timesteps') else 10_000_000
