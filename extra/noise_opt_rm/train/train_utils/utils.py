...
def build_noise_library(cfg):
    ...
    all_noises = np.load(noise_samples_path)
    if all_noises.ndim == 3 and all_noises.shape[1] == action_horizon and all_noises.shape[2] == action_dim:
        all_noises = all_noises.reshape(all_noises.shape[0], flat_dim)
    elif all_noises.ndim != 2:
        raise ValueError(f"Unexpected noise array shape {all_noises.shape}; expected (N,{action_horizon},{action_dim}) or (N,{flat_dim})")
    
    noise_lib = all_noises[:top_k].astype(np.float32)
    print(f">>> Loaded top-{top_k} noises from {noise_samples_path}")
    return noise_lib
...
def collect_rollouts(model, env, num_steps, base_policy, cfg):
    ...
    total_steps = num_steps * cfg.env.n_envs * cfg.act_steps
...
class CheckpointCallbackOnEpisodes(BaseCallback):
    ...
    def _checkpoint_path(self, episode_num: int, timesteps: int, checkpoint_type: str = "", extension: str = "") -> str:
    ...
    def _on_step(self) -> bool:
        ...
        current_timesteps = (self.model.num_timesteps * self.action_chunk) + self.initial_steps

        # Check if we've reached a checkpoint milestone
        for checkpoint_ep in self.checkpoint_episodes:
            if current_episode_num >= checkpoint_ep and checkpoint_ep not in self.saved_checkpoints:
                self.saved_checkpoints.add(checkpoint_ep)
                
                model_path = self._checkpoint_path(checkpoint_ep, current_timesteps, extension="zip")
                self.model.save(model_path)
                if self.verbose >= 1:
                    print(f"Saving model checkpoint to {model_path}")

                # Save locals to JSON
                locals_path = self._checkpoint_path(checkpoint_ep, current_timesteps, extension="json")
                ...
                if self.save_replay_buffer and hasattr(self.model, "replay_buffer") and self.model.replay_buffer is not None:
                    ...
                    replay_buffer_path = self._checkpoint_path(checkpoint_ep, current_timesteps, "replay_buffer_", extension="pkl")
                ...
                if self.save_vecnormalize and self.model.get_vec_normalize_env() is not None:
                    ...
                    vec_normalize_path = self._checkpoint_path(checkpoint_ep, current_timesteps, "vecnormalize_", extension="pkl")
