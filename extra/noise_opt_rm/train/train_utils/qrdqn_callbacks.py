...
class QRDQNNoiseLoggingCallback(BaseCallback):
    """Separate logging callback for QR-DQN so original DSRL callback remains untouched."""
    def __init__(self,
                 action_chunk=4,
                 log_freq=1000,
                 use_wandb=True,
    ...
    def _on_step(self):
        ...
        if self.n_calls % self.log_freq == 0 and len(self.episode_rewards) > 0:
            if self.use_wandb:
                ...
                for k in ['train/loss', 'train/n_updates']:
