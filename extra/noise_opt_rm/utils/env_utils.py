...
class BCRecorderEnvWrapper(VecEnvWrapper):
	"""Wrapper that records state, action, reward, success, and episode length for a single environment.
	Note that the data is stored as s_t, a_t, r_t."""
	def __init__(self, env, save_dir=None, cfg=None):
		super().__init__(env)
		assert self.num_envs == 1, "BCRecorderEnvWrapper only supports num_envs=1"
		...
		self.normalize = cfg is not None and hasattr(cfg, 'normalization_path') and cfg.normalization_path is not None
		if self.normalize:
	...
	def reset(self, **kwargs):
		...
		self.episode_states.append(obs[0])
		return obs

	def _save_episode_data(self):
		"""Save episode data to an npy file."""
		if self.save_dir is None:
			return	
		assert len(self.episode_states) == len(self.episode_actions) \
			== len(self.episode_rewards) == self.episode_length, \
				"Mismatch in episode data lengths"

		episode_data = {
			'states': np.array(self.episode_states),
			'actions': np.array(self.episode_actions) if self.episode_actions else None,
			'rewards': np.array(self.episode_rewards),
			'reward': float(self.episode_reward),
			'success': bool(self.episode_success),
			'length': int(self.episode_length),
			'infos': self.episode_infos
		}
		
		filename = f'episode_{self.episode_counter}.npy'
		filepath = os.path.join(self.save_dir, filename)
		np.save(filepath, episode_data, allow_pickle=True)
		self.episode_counter += 1

	def step_async(self, actions):
		# Unnormalize action before recording
		action_to_record = self.unnormalize_action(actions[0])
		self.episode_actions.append(action_to_record) # assuming single env
	...
	def step_wait(self):
		...
		self.episode_infos.append(infos[0])
		self.episode_length += 1
		if 'is_success' in infos[0]:
			self.episode_success = self.episode_success or bool(infos[0]['is_success'])
		if dones[0]:
			# Remove the last obs above since SB3 resets after done
			# And also since it is the terminal obs for recording BC data
			if len(self.episode_states) > 0:
				self.episode_states.pop()
			infos[0]['episode'] = {
				'reward': float(self.episode_reward),
				'success': bool(self.episode_success),
				'length': int(self.episode_length)
...
def build_single_env(policy, cfg, video_dir, seed, save_vid=True, record_dir=None):
	...
	if record_dir is not None: # needs to sit under diffpo wrapper to get denoised actions
		env = BCRecorderEnvWrapper(env, save_dir=record_dir, cfg=cfg)
	env = DiffusionPolicyEnvWrapper(env, cfg, policy)
	env.seed(seed)
	return env
