...
def evaluate_noise_random(env, noise_vecs, save_vid=False, eval_num=0, rew_offset=1.0, initial_obs=None):
	"""Randomly sample from noise_vecs at every timestep. Success if any step reward > -rew_offset."""
	if save_vid:
		env.env.name_prefix = f"random_eval_{eval_num}"
	...
	noise_indices = list(noise_vecs.keys())

	steps = 0
	while not done:
		idx = random.choice(noise_indices)
		action = noise_vecs[idx].reshape(1, -1)
		_, r, d, info = env.step(action)
		steps += 1
		r_val = float(r[0])
		episode_reward += r_val
...
def load_noise_idx(eval_path: str, eval_idx: int = 0):
	"""Load noise vector at eval_idx from noise_samples.npy in eval_path or directly from .npy file."""
	# Check if eval_path is a .npy file
	if eval_path.endswith('.npy'):
		noise_path = eval_path
	else:
		noise_path = os.path.join(eval_path, "noise_samples.npy")
	
	if not os.path.exists(noise_path):
		raise FileNotFoundError(f"No noise file found at {noise_path}")
