...
RobomimicLowdimWrapper (DPPO) -> ObservationWrapperRobomimic (DSRL, parent: gym.Env) -> ActionChunkWrapper (p: gym.Env) -> 
VecEnv (SB3) -> DiffusionPolicyEnvWrapper (p: VecEnvWrapper- SB3)
...
from noise_opt_rm.eval.eval_utils import evaluate_noise_single, evaluate_noise_random, load_noise_idx, save_eval_serial
...
def p_args():
	...
	p.add_argument("--eval", type=str, nargs='+', default=None, help="List of paths to noise folders")
	p.add_argument("--eval_idx", type=int, nargs='+', default=[0], help="List of noise indices to evaluate")
	p.add_argument("--random_sample", action="store_true", help="Randomly sample from eval_idx at every timestep")
...
def main():
	...
	if args.eval:
		if len(args.eval) == 1:
			ticket_name = os.path.basename(args.eval[0].rstrip('/'))
		else:
			ticket_name = f"multi_path_{len(args.eval)}paths"
	else:
		ticket_name = None
	...
	# If random_sample is enabled, evaluate once with mixed sampling
	if args.random_sample:
		eval_indices = [args.eval_idx[0]]  # Loop once for random sampling
		noise_vecs = {}
		# Load noise vectors from all paths and indices
		for eval_path in args.eval:
			for idx in args.eval_idx:
				noise_key = f"{os.path.basename(eval_path.rstrip('/'))}_idx{idx}"
				noise_vecs[noise_key] = load_noise_idx(eval_path, idx).astype(np.float32).flatten()
		print(f">>> Loaded {len(noise_vecs)} noise vectors from {len(args.eval)} paths for random sampling")
		print(f">>> Noise keys: {list(noise_vecs.keys())}")
	else:
	...
	for eval_idx in eval_indices:
		print(f"\n{'='*80}")
		print(f"Evaluating noise index: {eval_idx} ({eval_indices.index(eval_idx) + 1}/{len(eval_indices)})")
		print(f"{'='*80}")
		...
		if args.random_sample:
			args.out = os.path.join(base_out, f"random_sample")
		else:
			best_noise = load_noise_idx(args.eval[0], eval_idx)
			print(f">>> Loaded noise idx {eval_idx} from {args.eval[0]}")
		...
		for seed_idx in range(args.n_seeds):
			...
			for eval_iter in range(args.n_evals_per_seed):
				...
				if args.random_sample:
					episode_reward, success = evaluate_noise_random(
						env, noise_vecs, save_vid, eval_num=eval_iter, rew_offset=cfg.env.reward_offset
					)
				else:
	...
	if len(eval_indices) > 1 and not args.random_sample:
		reward_means = [all_indices_results[idx]["reward_mean"] for idx in eval_indices]
		success_means = [all_indices_results[idx]["success_mean"] for idx in eval_indices]
