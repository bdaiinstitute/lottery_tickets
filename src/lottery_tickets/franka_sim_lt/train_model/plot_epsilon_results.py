...
def load_ticket_results(
    results_path: Path, success_threshold: float
) -> TicketResult | None:
...
def plot_ticket_success(axes: Axes, results: TicketResult) -> None:
    ...
    axes.scatter(
        results.epsilons,
        100 * np.count_nonzero(results.successes, axis=1) / results.successes.shape[1],
...
def main(
    results_root: Path, output_dir: Path, success_threshold: float = 100.0
) -> None:
    ...
    for ticket_dir in results_root.iterdir():
        if (
            ticket_result := load_ticket_results(ticket_dir, success_threshold)
        ) is not None:
