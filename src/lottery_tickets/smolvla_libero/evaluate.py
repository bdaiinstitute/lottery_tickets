...
For our experiments, we use LeRobot's finetuned version of SmolVLA for LIBERO: "HuggingFaceVLA/smolvla_libero" 
...
Generate a new noise vector (it will be sampled from standard normal), and run eval on it. The noise vector will be saved to output_dir/A_UNIQUE_ID/initial_noise.pt for future use, along with videos and results.
...
def eval_main(cfg: EvalPipelineConfigNoisePath):
    ...
    ### Add fixed inital noise for the entire eval
    # For sanity checking, make sure batch size is just 1 for now, just stack it later for multiple envs
    assert cfg.eval.batch_size == 1, "Eval batch size must be 1 for fixed noise eval"
    # make a generate so that each initial noise is different across different eval runs
    noise_rng = torch.Generator(device=cfg.policy.device)
    noise_rng.manual_seed(int(time.time() * 1000) % 2**32)
    ...
    # Save noise vector
    if noise != None:
        torch.save(noise,  output_dir / "initial_noise.pt")
