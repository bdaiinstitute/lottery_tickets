#!/bin/bash
# Submit evaluation jobs for all object tickets

TICKET_DIR="/home/opatil3/src/lottery_tickets/src/lottery_tickets/smolvla_libero/new_golden_tickets/libero_object_tickets"
...
for ticket_folder in "$TICKET_DIR"/*/; do
    ...
    if [[ -f "$noise_path" ]]; then
        ...
        sbatch -J "ob_$ticket_hash" lt_eval_object.sh "$noise_path"
    else
